...
Deno.serve(async(req:Request)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:H});if(req.method!=='POST')return res({ok:false,code:'method_not_allowed'},405);try{
  ...
  const b=await req.json().catch(()=>({})),current=Number(b.currentWeight),target=Number(b.targetWeight),targetDate=String(b.targetDate||''),weeks=weeksTo(targetDate),rate=(current-target)/weeks;
  if(!Number.isFinite(current)||!Number.isFinite(target)||!/^\d{4}-\d{2}-\d{2}$/.test(targetDate))return res({ok:false,code:'invalid_goal'},400);
  if(rate>1)return res({ok:false,code:'goal_too_aggressive',requestedKgPerWeek:+rate.toFixed(2),message:'Bitte den Zieltermin verlängern. Fitnest erzeugt keinen Crash-Plan über 1 kg/Woche.'},422);
  ...
  if(!r.ok){console.error('openai_training',r.status,await r.text());return res({ok:true,source:'rules-fallback',model:null,weeks,rate:+rate.toFixed(2),sessions:fallback(days,minutes),coachNote:'KI war kurzfristig nicht verfügbar. Fitnest nutzt einen konservativen Ersatzplan.'})}
  const j=await r.json(),text=outputText(j);if(!text)return res({ok:true,source:'rules-fallback',model:null,weeks,rate:+rate.toFixed(2),sessions:fallback(days,minutes),coachNote:'KI lieferte keine verwertbare Antwort. Fitnest nutzt einen konservativen Ersatzplan.'});
  const parsed=JSON.parse(text),sessions=normalize(parsed,days,minutes);return res({ok:true,source:'openai',model:MODEL,weeks,rate:+rate.toFixed(2),sessions,coachNote:String(parsed.coachNote||'').slice(0,500)});
