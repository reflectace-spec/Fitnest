...
assert.match(version.build,/^3\.8\.[6-9]$/);
assert.equal(manifest.start_url,'./');
assert.match(index,/app\.js\?v=3\.8\.[6-9]/);
assert.match(index,/update-bootstrap\.js\?v=3\.8\.[6-9]/);
assert.ok((app.match(/\?v=3\.8\.[6-9]/g)||[]).length>=35);

assert.match(recipes,/import \{ CONFIG \} from '\.\/config\.js';/);
assert.match(recipes,/CONFIG\.supabaseUrl/);
assert.match(settings,/const BUILD='3\.8\.[6-9]'/);
assert.match(pwa,/const BUILD='3\.8\.[6-9]'/);
assert.match(pwa,/\[data-v35-center-open\]/);
assert.match(tutorial,/const BUILD='3\.8\.[6-9]'/);
assert.match(nativeShell,/const BUILD='3\.8\.[6-9]'/);
assert.match(changelog,/version:'Build 3\.8\.6'/);
assert.match(changelog,/Kanonische Produktion und vollständiger App-Start/);

assert.match(bootstrap,/const BUILD='3\.8\.[6-9]'/);
...
assert.match(sw,/fitnest-shell-v3-8-[6-9]/);
