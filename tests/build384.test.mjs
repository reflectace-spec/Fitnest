...
assert.match(index,/update-bootstrap\.js\?v=3\.8\.[4-9]/);
assert.match(index,/app\.js\?v=3\.8\.[4-9]/);
assert.match(index,/build383\.css\?v=3\.8\.[4-9]/);
assert.ok((app.match(/\?v=3\.8\.[4-9]/g)||[]).length>=35,'feature modules must be versioned');
assert.match(changelog,/const BUILD='3\.8\.[4-9]'/);
assert.match(changelog,/Zuverlässige automatische App-Updates/);
assert.match(pwa,/const BUILD='3\.8\.[4-9]'/);
