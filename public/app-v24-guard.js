...
function syncTodaySummary(){if(document.querySelector('.tab.active')?.dataset.view!=='today')return;const p=activeProfile(),summary=app?.querySelector('[data-nsum] .nutrition-summary');if(!p||!summary)return;const plan=read('fitnest.nutrition.plans',{})[iso()],logs=read('fitnest.nutrition.logs',{})[iso()]||{},meals=plan?.profileId===p.id?plan.meals||[]:[],entries=Object.values(logs),kcal=entries.reduce((s,x)=>s+(+x.calories||0),0),protein=entries.reduce((s,x)=>s+(+x.protein_g||0),0);summary.innerHTML=`<div><strong>${entries.length}/${meals.length||p.mealsPerDay} Mahlzeiten</strong><small>${kcal}/${p.calories} kcal · ${Math.round(protein)}/${p.protein} g Protein · ${p.name}</small></div><span class="pill">${entries.length===(meals.length||p.mealsPerDay)?'Erledigt':'Heute'}</span>`}
